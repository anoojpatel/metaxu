...
```
```output
ptr_read: out of bounds (<*heap#1>) — index 2, size 2
```
...
```
```output
ptr_write: readonly snapshot (<*heap#1>)
hi
```
...
`is_null` is the test; there is no null anywhere else in the
language, so the check lives here at the boundary and doesn't leak
out. The pattern for wrapping any C function is the same as wrapping
