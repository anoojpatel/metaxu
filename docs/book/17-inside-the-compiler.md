...
A type conflict reads as the two facts that can't both hold, with
the caret on the value they collided over:
...
```
```output
<mem>:2:13: type check failed: one value is required to be Int and String
    let n = 1 + "a";
            ^
```
