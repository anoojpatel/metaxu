...
F64 = "f64"
STR = "str"
# f32 (docs/gpu_tiles.md Stage 1d) is a TILE ELEMENT kind only: it appears
# inside `tile:f32:RxC` kind strings and NEVER as a value kind of its own —
# language scalars stay f64, and every scalar/Vec boundary of an f32 tile
# (sum, get, scale's factor, to_vec, stores) is f64 (the elements are
# stored as f32-representable doubles, so widening is exact and free).
F32 = "f32"
# f16 (Stage 1f) follows the identical rule: tile-element-only, stored as
# the f16-representable double, every scalar/Vec boundary crosses as f64.
F16 = "f16"
...
# and `from_bytes` decodes one back (rejecting non-bytes, NUL and invalid
# UTF-8 with the interpreter's diagnostics).
_STR_BUILTIN_SIGS = {
    "find": ("mx_str_find", (STR, STR), I64),
    "split": ("mx_str_split", (STR, STR), _VEC_PREFIX + STR),
    "replace": ("mx_str_replace", (STR, STR, STR), STR),
    "trim": ("mx_str_trim", (STR,), STR),
    "join": ("mx_str_join", (_VEC_PREFIX + STR, STR), STR),
    "to_bytes": ("mx_str_to_bytes", (STR,), _VEC_PREFIX + I64),
    "from_bytes": ("mx_bytes_to_str", (_VEC_PREFIX + I64,), STR),
}
...
_RT_SIGS = {
    ...
    "mx_str_to_bytes": ("ptr", ("ptr",)),
    "mx_bytes_to_str": ("ptr", ("ptr",)),
...
def _tile_scalar(elem: str) -> str:
    """The LANGUAGE kind of one element at a scalar/Vec boundary: f32/f16
    elements cross as f64 (they are stored as representable doubles)."""
    return F64 if elem in (F32, F16) else elem
...
def _join(a: str, b: str) -> str:
    ...
    if _is_tile(a) and _is_tile(b):
        # Shapes AND element kinds are part of the kind: tiles arise
        # fully-formed from their ctor (there is no elem bottom), so a
        # mismatch on either never joins — letting i64 promote here would
        # reinterpret int words as doubles (wrong output) where the
        # interpreter raises; a CONFLICT demotes honestly instead.
        ea, ra, ca = _tile_parts(a)
        eb, rb, cb = _tile_parts(b)
        if (ra, ca) != (rb, cb) or ea != eb:
            return CONFLICT
        return a
...
def _resolve_trait_call(method: str, recv_kind: str, traits: _TraitTable,
    ...
    if _is_vec(recv_kind):
        ...
        if hit is not None:
            return hit
        if method in ("push", "pop", "len", "join", "from_bytes"):
    ...
    if recv_kind == STR:
        ...
        if hit is not None:
            return hit
        if method in ("len", "find", "split", "replace", "trim", "to_bytes"):
            return ("builtin", method)
        if method in ("to_string", "int_to_str"):
            return ("builtin", "to_string")
        return plain_fn_fallback(
...
def _infer_kinds(info: _Info, sigs: Dict[str, _Sig], structs: _StructTable,
    ...
    def apply_builtin(name: str, dst: str, args: Tuple[str, ...],
        ...
        if name == "Vec.new":
            ...
            # programs and a disagreement conflicts.
    ...
    def tile_builtin(op: str, dst: str, args: Tuple[str, ...]) -> bool:
        ...
        if op in ("zeros", "arange") and len(args) == 2:
            ...
            if _is_tile(get(args[0])):
                ...
                ch = mark(dst, _vec_of(_tile_scalar(elem))) or ch
            ...
            if _is_tile(get(args[0])):
                elem, _r, _c = _tile_parts(get(args[0]))
                ch = mark(args[1], _tile_scalar(elem)) or ch
            ...
            if _is_tile(get(args[0])):
                elem, _r, _c = _tile_parts(get(args[0]))
                ch = mark(dst, _tile_scalar(elem)) or ch
            ...
            if _is_tile(get(args[2])):
                ...
                ch = mark(args[0], _vec_of(_tile_scalar(elem))) or ch
            # dst is unit -> stays i64
            ...
            if _is_tile(get(args[3])):
                ...
                ch = mark(args[0], _vec_of(_tile_scalar(elem))) or ch
            # dst is unit -> stays i64
        elif op in ("to_f32", "to_f16", "to_f64") and len(args) == 1:
            # Total, shape-preserving conversions: the result's element
            # kind is fixed by the op, the shape rides over from the
            # receiver (docs/gpu_tiles.md Stage 1d/1f).
            if _is_tile(get(args[0])):
                _e, r, c = _tile_parts(get(args[0]))
                ch = mark(dst, _tile_of(
                    {"to_f32": F32, "to_f16": F16,
                     "to_f64": F64}[op], r, c)) or ch
...
def _check_consistency(info: _Info, kinds: Dict[str, str], sigs: Dict[str, _Sig],
    ...
    def check_builtin(name: str, dst: str, args: Tuple[str, ...]) -> None:
        ...
        if name == "Vec.new":
            ...
            if len(args) != len(pks):
                probs.append(
                    f"{name} with {len(args)} arguments (expects {len(pks)})")
            else:
                for a, want in zip(args, pks):
                    if ty(a) != want:
                        probs.append(
            ...
            if len(args) != 1:
                ...
                if _tile_parts(ak)[0] not in (I64, F64, F32, F16):
                    probs.append(
            ...
            if top in ("zeros", "filled", "arange", "from_vec", "load",
                ...
                vk = ty(args[0])
                if elem not in (I64, F64, F32, F16):
                    probs.append(f"{name} of {elem} elements "
                                 "(tiles hold int or float)")
                elif not _is_vec(vk):
                    probs.append(f"{name} target {args[0]!r} has kind {vk}, "
                                 "not a Vec")
                elif _vec_elem(vk) != _tile_scalar(elem):
                ...
                if top in ("add", "mul"):
                    ...
                    if ty(args[1]) != _tile_scalar(elem):
                    ...
                    elif ty(dst) != _tile_scalar(elem):
                        probs.append(f"{name} result {dst!r} is {ty(dst)}, "
                                     f"elements are {elem}")
                elif top == "to_vec":
                    if ty(dst) != _vec_of(_tile_scalar(elem)):
                        probs.append(f"{name} result {dst!r} is {ty(dst)}, "
                                     f"expected {_vec_of(_tile_scalar(elem))}")
                elif top in ("to_f32", "to_f16", "to_f64"):
                    want_k = _tile_of({"to_f32": F32, "to_f16": F16,
                                       "to_f64": F64}[top], r, c)
                    if ty(dst) != want_k:
                        probs.append(f"{name} result {dst!r} is {ty(dst)}, "
                                     f"expected {want_k}")
...
_VEC_SAFE_RECEIVER_BUILTINS = {"push", "pop", "len", "__index_get", "as_ptr",
                               # mx_str_join / mx_bytes_to_str copy the
                               # elements out and retain nothing
                               "join", "from_bytes"}
        ...
        for op in b.ops:
            ...
            if is_str_concat(rhs, args, dst):
                ...
                if callee in info.def_count:
                    ...
                    elif bname in _STR_BUILTIN_SIGS:
...
def _emit_function(info: _Info, kinds: Dict[str, str], sigs: Dict[str, _Sig],
    ...
    def emit_rt_builtin(name: str, dst: str, opargs: Tuple[str, ...],
        ...
        if name == "Vec.new":
            ...
            # call; the `tile:` kind supplies the element type (ekind
            # code: 0 int, 1 f64, 2 f32, 3 f16) and the shapes are already
            # burned into the kinds the consistency check validated.
            # Blocks are write-once and leak by design, like fvec.
            top = name[len("Tile."):]

            def tflag(k: str) -> str:
                """The runtime ekind code for a tile kind's elements."""
                return {I64: "0", F64: "1", F32: "2",
                        F16: "3"}[_tile_parts(k)[0]]
            ...
            if top in ("zeros", "arange"):
                ...
                a = use(opargs[0], lines)
                w = to_word(_tile_scalar(elem), use(opargs[1], lines), lines)
                ...
                vec_elem_into(dst, _tile_scalar(elem), w, lines)
            elif top == "transpose":
                a = use(opargs[0], lines)
            ...
            if k == STR:
                ...
                lines.append(
                    ...
                    f"i64 {0 if elem == I64 else 1})"
                    "  ; tile repr (fresh string, leaks by design; f32/"
                    "f16 elements ARE doubles, so they format as f64)")
                setval(dst, v, lines)
            elif _is_fvec(k):
                ...
                if callee in info.def_count:
                    ...
                    def tile_repr(a: str) -> str:
                        ...
                        lines.append(
                            ...
                            f"ptr {use(a, lines)}, "
                            f"i64 {0 if elem == I64 else 1})"
                            "  ; tile repr for print (f32/f16 elements "
                            "ARE doubles)")
                        return sv
                    fvec_temps: List[str] = []
                    if len(opargs) == 1:
